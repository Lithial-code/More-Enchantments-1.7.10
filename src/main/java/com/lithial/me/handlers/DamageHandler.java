        ...
        if (event.entityLiving instanceof EntityLiving)
        {
            ...
            if (event.source.getEntity() instanceof EntityPlayer) {
                ...
                if(player instanceof EntityPlayer)
                    player.addChatComponentMessage(new ChatComponentText("player did " +event.ammount + " damage" ));
                if(Enchantments.allowMulti) {
                    ...
                    if(Utils.getEnchHelp(player,Enchantments.purging.effectId,player.getCurrentEquippedItem())!=0) {
                        if(living.isImmuneToFire())
                        {
                            event.ammount += (Utils.getEnchHelp(player, Enchantments.purging.effectId, player.getCurrentEquippedItem()) *Enchantments.purgingBonus);
                        /*    player.addChatComponentMessage(new ChatComponentText("player did a critical"));
                            player.addChatComponentMessage( new ChatComponentText("bonus damage  " + event.ammount));*/
                        }
                    }
                ...
                if(event.source.isProjectile())
                {
                    if(Enchantments.allowRD) {
                        ...
                        if (living instanceof EntityCreeper) {
                            world.createExplosion(living, living.posX, living.posY, living.posZ, (float) 3, flag);
                            living.setDead();
                        }
                    }
                    if(Utils.getEnchHelp(player,Enchantments.demonhunter.effectId,player.getCurrentEquippedItem())!=0) {
                        if(living.isImmuneToFire())
                        {
                            event.ammount += (Utils.getEnchHelp(player, Enchantments.demonhunter.effectId, player.getCurrentEquippedItem()) *Enchantments.demonBonus);
                        /*    player.addChatComponentMessage(new ChatComponentText("player did a critical"));
                            player.addChatComponentMessage( new ChatComponentText("bonus damage  " + event.ammount));*/
                        }
                    }
